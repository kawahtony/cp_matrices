...
for k = 1:length(band)
    Htemp = [Hx(k); Hy(k); Hz(k)] ;
    ntemp = [nx(k); ny(k); nz(k)] ;
    Htemp = Htemp - dot(Htemp, ntemp)*ntemp ;
    Hx(k) = Htemp(1) ; Hy(k) = Htemp(2) ; Hz(k) = Htemp(3) ;
end
...
while t < T
    ...
    for k = 1:numpt
        ...
        fmax = max(fmax, norm(f(x,k) - kappa*(Etemp*[Hx, Hy, Hz])')) ;
    end
    ...
    for k = 1:numpt
        ...
        k1_x(:,k)= ftemp - kappa*(Etemp*[Hx, Hy, Hz])' ;
...
filename = ['nbody_ellipsoid_damped_curvature_num=', num2str(numpt), '.mat'] ;
